	...
	$.ajax({
		...
		success: function ( respuesta )
		{
			if (respuesta.resultado)
			{
				...
				$("#modal-editar #form-editar #contacto").val(respuesta.registros.contacto);
...
function eliminarRegistro( e )
{
	...
	swal({
		...
		if ( isConfirm )
		{
			$.ajax({
				...
				success: function ( respuesta )
				{
					if( respuesta.resultado )
					{
						swal("Listo!", "El Proveedor fue eliminada", "success");
						setTimeout( function(){ ratPack.refresh(); }, 300 );
					}
					else
					{
		...
		{
			swal("Cancelado!", "Ya no se eliminara el Proveedor", "error");
		}
	});
