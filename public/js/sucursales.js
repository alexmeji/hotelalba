	...
	$.ajax({
		...
		success: function ( respuesta )
		{
			if (respuesta.resultado)
			{
				...
				//$("#modal-editar #form-editar #select-editar").val(respuesta.registros.idmunicipio).triggerHandler('update');;
				
				var select = $("#form-editar #select-editar").selectize();
				var selectize = select[0].selectize; 
				selectize.setValue(respuesta.registros.idmunicipio);
			}
			else
			{
				nGen('warning', 'fa fa-times-circle', 'Espera..!', respuesta.mensaje , 'topRight');		
...
function eliminarRegistro( e )
{
	...
	swal({
		...
		if ( isConfirm )
		{
			$.ajax({
				...
				success: function ( respuesta )
				{
					if( respuesta.resultado )
					{
						swal("Listo!", "La Sucursal fue eliminada", "success");
						setTimeout( function(){ ratPack.refresh(); }, 300 );
					}
					else
					{
		...
		{
			swal("Cancelado!", "Ya no se eliminara la Sucursal", "error");
		}
	});
