	...
	$.ajax({
		...
		success: function ( respuesta )
		{
			if (respuesta.resultado)
			{
				...
				$("#modal-editar #form-editar #precio").val(respuesta.registros.precio);
...
function eliminarRegistro( e )
{
	...
	swal({
		...
		if ( isConfirm )
		{
			$.ajax({
				...
				success: function ( respuesta )
				{
					if( respuesta.resultado )
					{
						swal("Listo!", "La Tarifa fue eliminada", "success");
						setTimeout( function(){ ratPack.refresh(); }, 300 );
					}
					else
					{
		...
		{
			swal("Cancelado!", "Ya no se eliminara la Tarifa", "error");
		}
	});
