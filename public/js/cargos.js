	...
	$.ajax({
		...
		success: function ( respuesta )
		{
			if (respuesta.resultado)
			{
				...
				$("#modal-editar #form-editar #costo").val(respuesta.registros.costo);
...
function eliminarRegistro( e )
{
	...
	swal({
		title: "¿Estas seguro?",
		text:  "Eliminaras el Tipo de Cargo",
		type:  "warning",
		...
		if ( isConfirm )
		{
			$.ajax({
				...
				success: function ( respuesta )
				{
					if( respuesta.resultado )
					{
						swal("Listo!", "El Tipo de Cargo fue eliminado", "success");
						setTimeout( function(){ ratPack.refresh(); }, 300 );
					}
					else
					{
		...
		{
			swal("Cancelado!", "Ya no se eliminara el Tipo de Cargo", "error");
		}
	});
